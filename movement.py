...
class Movement:
    ...
    def movement(self, grid: Grid, selected_cell: Cell, target_cell: Cell) -> Grid:
        ...
        if grid.grid[target_cell.x][target_cell.y].type in ['p', 'wp']:
            self.movement_cell_by_purple(grid.grid[target_cell.x][target_cell.y], grid)
    ...
    def movement_cell_by_purple(self, selected_cell, grid: Grid):
        ...
        # دالة لمعالجة الحركة في اتجاه معين
        def process_direction(dx, dy, direction):
            stack = []
            found_movable_cell = False
            i, j = x + dx, y + dy
            # التكرار عبر الخلايا في الاتجاه المحدد
            ...
            # تنفيذ الحركة عبر الخلايا المضافة إلى الستاك
            if stack:
                while stack:
        ...
        # تنفيذ الحركة في الاتجاهات الأربعة
        process_direction(-1, 0, 'up')   # الحركة للأعلى
        process_direction(1, 0, 'down')  # الحركة للأسفل
        process_direction(0, 1, 'right')  # الحركة لليمين
        process_direction(0, -1, 'left')  # الحركة لليسار

    def movement_by_red(self, selected_cell, grid: Grid):
        x, y = selected_cell.x, selected_cell.y

        # تحديد الاتجاهات الممكنة للحركة
        directions = [
            (0, 1),   # Right
            (0, -1),  # Left
            (-1, 0),  # Up
            ...
            if 0 <= i < grid.row and 0 <= j < grid.col and grid.grid[i][j].type not in ['b', 'wb']:
                ...
                while 0 <= i < grid.row and 0 <= j < grid.col:
                    ...
                    if first_b_cell and before_first_b_cell:
                        switched_b = move_and_switch(
                            current_cell, first_b_cell, before_first_b_cell, switched_b)

                    # الانتقال للخلية التالية في نفس الاتجاه
                    i += dx
                    j += dy

        def move_and_switch(current_cell, first_b_cell, before_first_b_cell, switched_b):
            # تحقق من نوع الخلية وقم بالتبديل وفقًا لذلك
